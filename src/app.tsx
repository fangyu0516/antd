...
const initAuthorization = { authorization: undefined || '' };
...
export async function getInitialState(): Promise<{
  ...
  // 如果不是登录页面,则吧currentUser放进去,针对的是关了页面后再打开，还有自动登陆
  ...
  return {
    ...
    onPageChange: () => {
      ...
      if (!localStorage.getItem('authorization')) {
        if (initialState) initialState.currentUser = undefined;
      }
      // 如果没有登录，重定向到 login
...
export const request: RequestConfig = {
  errorHandler,
  // 新增自动添加AccessToken的请求前拦截器,在网络请求的 .then 或 catch 处理前拦截，
  requestInterceptors: [authHeaderInterceptor],

  // //响应后拦截,在网络请求响应的 .then 或 catch 处理前拦截处理
